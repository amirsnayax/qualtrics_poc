...
class QualtricsManagerImpl implements QualtricsOps {
  ...
  Future<TaskEither<Exception, Map<String, String>>> initializeProject(
    ...
    print("initializeProject: $params");
    return TaskEither.tryCatch(
        () => qualtrics.initializeProject(params.brandId, params.projectId).then((value) {
          print("initializeProject: $value");
          return value;
        }),
  ...
  Future<TaskEither<Exception, Map<String, String>>> evaluateIntercept(
    ...
    print("evaluateIntercept: $interceptId");
    return TaskEither.tryCatch(
        () => qualtrics.evaluateIntercept(interceptId).then((value) {
          print("evaluateIntercept: $value");
  ...
  Future<TaskEither<Exception, bool>> displayIntercept(
    ...
    print("displayIntercept: $interceptId");
    return TaskEither.tryCatch(
      () => qualtrics.displayIntercept(interceptId).then((value) {
        print("displayIntercept: $value");
        return value;
      }),
      (error, stackTrace) => Exception(error.toString()),
    );
